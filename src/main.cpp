...
//TODO: CENTER FRAMEBUFFER
//TODO: CREATE CUSTOM SKYBOX
...
int load_obj_file(const std::string &file, std::vector <glm::vec3> &Vertices, std::vector <glm::vec3> &Normals, std::vector <glm::vec3> &Texcoords, std::vector<glm::vec3> &Tangents, std::vector<unsigned int> &Indices) {
	...
	for(int i = 0; i < mesh->mNumVertices; i++) {
		...
		if(mesh->HasNormals()) {
			...
			aiVector3D default_normal(0.0f, 1.0f, 0.0f);
			Normals.push_back(glm::vec3(default_normal.x, default_normal.y, default_normal.z));
		}
...
void captureScene(int screenshot_number, int win_width, int win_height) {
	...
	char name[55];
	snprintf(name, sizeof(name), "screenshot%d.ppm", screenshot_number);
	FILE *f = fopen(name, "w");
	fprintf(f, "P3\n%d %d\n%d\n", win_width, win_height, 255);
	for(int i = 0; i < win_height; i++) {
		for(int j = 0; j < win_width; j++) {
			size_t pix = 3 * ((win_height - i - 1) * win_width + j);
...
int main() {
	...
	int cube_v_total = load_obj_file("cube.obj", cube_vertices, cube_normals, cube_texcoords, cube_tangents, cube_indices);
	if(cube_v_total == -1) {
	...
	int die_v_total = load_obj_file("die.obj", die_vertices, die_normals, die_texcoords, die_tangents, die_indices);
	if(die_v_total == -1) {
		std::cerr << "Failed to load .obj file." << std::endl;
		return 1;
	}
	...
	int sphere_v_total = load_obj_file("sphere.obj", sphere_vertices, sphere_normals, sphere_texcoords, sphere_tangents, sphere_indices);
	if(sphere_v_total == -1) {
		std::cerr << "Failed to load .obj file." << std::endl;
		return 1;
	}
	...
	while(!glfwWindowShouldClose(window)) {
		...
		projection_info[0].aspect_ratio = aux::get_aspect_ratio(win_width, win_height);
		...
		if(win_height == win_width) { //I want to keep the 1:1 aspect ratio
			...
			glViewport(win_width/4, 0, win_height, win_height);
		} else {
			glViewport(0, win_height/4, win_width, win_width);
		}
		...
		//TAKE SCREENSHOT
		if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS && s_key_pressed == false) { //WOW glfw callbacks are awful, even lambda cant fix them, for OOP only pretty much
			s_key_pressed = true;
